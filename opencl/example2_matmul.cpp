...
//    if (clGetDeviceIDs(platform_id, CL_DEVICE_TYPE_CPU, 1, &device_id, &n_devices) != CL_SUCCESS)
    ...
    // it should be useless in this case, but I've seen it gives quite a
    // performance boost.
    ...
    // if we use this, the performance boost will occur when matrix sizes
    // are multiple of the magic number.

    size_t group_pattern[] = {15,15,0};
