...
"	l_in[l_ii*(BLOCK+2) + l_jj] = m_in[i*size_j+j];"

	// skip edges
"	if(i != 0 && j != 0 && i != size_i-1 && j != size_j-1) {"
		// copy block edges in local mem
"		if(!ii)"
"			l_in[/*(l_ii-1)*(BLOCK+2) +*/ l_jj] = m_in[(i-1)*size_j+j];"
"		if(ii == BLOCK-1)"
"			l_in[(l_ii+1)*(BLOCK+2) + l_jj] = m_in[(i+1)*size_j+j];"
"		if(!jj)"
"			l_in[l_ii*(BLOCK+2) /* + l_jj-1*/] = m_in[i*size_j+j-1];"
"		if(jj == BLOCK-1)"
"			l_in[l_ii*(BLOCK+2) + l_jj+1] = m_in[i*size_j+j+1];"
"	}"
...
"	if(i != 0 && j != 0 && i != size_i-1 && j != size_j-1) {"
		// compute result
"		m_out[i*size_j + j] = l_in[l_ii*(BLOCK+2) + l_jj]*(1.0-4.0*discr*p) + discr*p*("
