...
"	int l_ii = (ii+1)*(BLOCK+2);"
"	int l_jj = jj+1;"

"	local double l_in[(BLOCK+2)*(BLOCK+2)];"

	// copy block in local mem
"	l_in[l_ii + l_jj] = m_in[ip+j];"

	// skip edges
"	if(i != 0 && j != 0 && i != size_i-1 && j != size_j-1) {"
		// copy block edges in local mem
"		if(!ii)"
"			l_in[l_jj] = m_in[ip-size_j+j];"
"		if(ii == BLOCK-1)"
"			l_in[l_ii+BLOCK+2 + l_jj] = m_in[ip+size_j+j];"
"		if(!jj)"
"			l_in[l_ii] = m_in[ip+j-1];"
"		if(jj == BLOCK-1)"
"			l_in[l_ii + l_jj+1] = m_in[ip+j+1];"
"	}"
...
"	if(i != 0 && j != 0 && i != size_i-1 && j != size_j-1) {"
		// compute result
"		m_out[ip + j] = l_in[l_ii + l_jj]*(1.0-4.0*discr*p) + discr*p*("
"			l_in[l_ii-BLOCK-2 + l_jj] +"
"			l_in[l_ii+BLOCK+2 + l_jj] +"
"			l_in[l_ii + l_jj-1] +"
"			l_in[l_ii + l_jj+1]);"
"		}"
