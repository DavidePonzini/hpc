...
// compile with:
// gcc -O2 -lopenblas matmul_cblas_openblas.c
// or
// gcc -O2 -I openblas/include/ -L openblas/lib/ -lopenblas matmul_cblas_openblas.c
...
/* performance evaluation for all matrix size from nmin to nmax */
	for (n=nmin; n<=nmax; n += step) {
		...
		ttstart = (double)tstart.tv_sec*1.0e9 + (double)tstart.tv_usec;
