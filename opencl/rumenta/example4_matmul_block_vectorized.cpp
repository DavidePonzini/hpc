...
// the "magic number" 15, found experimentally on my AMD Radeon R9 390, is not
// multiple of the vectorization size (4).  we choose 16 as the block size.
...
"      /* row x column within block, vectorized */                      "
"      for (kk = 0; kk < BLOCK; kk += 4) {                              "
"          double4 a_v = (double4)(a_blk[ii*BLOCK + kk], a_blk[ii*BLOCK + kk+1],a_blk[ii*BLOCK + kk+2], a_blk[ii*BLOCK + kk+3]);"
"          double4 b_v = (double4)(b_blk[kk*BLOCK + jj],b_blk[(kk+1)*BLOCK + jj], b_blk[(kk+2)*BLOCK + jj], b_blk[(kk+3)*BLOCK + jj]);"
"          double4 p_v = a_v * b_v;                                     "
"          cc += p_v.s0 + p_v.s1 + p_v.s2 + p_v.s3;                     "
"      }                                                                "
